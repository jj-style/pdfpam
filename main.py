...
def main():
    args = sys.argv[1:]
    if len(args) < 3:
        print("usage: collator file.txt directory output", file=sys.stderr)
        exit(1)
    ...
    out = Path(output)
    if out.exists():
        print(f"warning: {out} already exists. Aborting to prevent overwriting data", file=sys.stderr)
        exit(1)
    ...
    for file,page_range in frd.items():
        if not out.exists(): # first run
            ...
            subprocess.run(shlex.split(f"pdftk {file} cat {page_range} output {tmp1.name}"))
